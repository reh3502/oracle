...
Recovery training cases R02 and R10 explicitly request the same category, chat, information, and voice layout that their `minecraft` grader requires. An empty server supplies no convention from which an exact room count could be inferred. Their rate-limit and missing-usage faults, state predicates, split, and seeds are unchanged. Earlier reports remain evidence for their original corpus, not passing observations on the revised requests. R19 has the same identified context gap but remains unchanged pending a separately documented revision; its old held-out failures must not be erased or treated as passes.
