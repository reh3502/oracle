...
This corrects missing context in the original A10/A15 training dataset, which expected information/chat purposes without presenting the documented conventions. Their previous failures remain historical invalid-context observations, not passes on the revised dataset. The corpus hash changes, and both repaired cases need fresh evaluation. Other cases, including all held-out cases, retain their original definitions and behavior. The new oracle checks two text purposes and one voice channel, effective player/staff/outsider access, fixed role and membership state, preservation of existing resources, duplicate identities, and repeat behavior. Unexpected member-specific access exceptions fail. An offline test executes and repeats the real plan/apply path and demonstrates that open information posting, inaccessible discussion/voice, public access, changed unrelated resources, and duplicate identities fail grading. The fixture uses the current raw-overwrite host contract; it does not claim support for a named audience-profile API.
