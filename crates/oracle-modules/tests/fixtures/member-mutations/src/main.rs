...
impl RpcHandler for Probe {
    async fn handle(
        ...
        match method.as_str() {
            "hello" => Ok(
                json!({"protocol_major":1,"protocol_minor":2,"manifest":serde_json::from_str::<Value>(include_str!("../manifest.json")).unwrap()}),
            ),
            ...
            "operation.invoke"
                if matches!(
                    ...
                    Some("member_probe" | "mutation_probe" | "read_probe" | "actor_probe")
                ) =>
            {
                if params["operation"] == "actor_probe" { return Ok(json!({"actor":params["member"],"guild":params["guild"]})); }
