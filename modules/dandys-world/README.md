...
Run hosting provides `/hostrun organized [name]`, `/hostrun casual [name]` and `/signup id [toon]`, alongside `/dw run id`, `/dw runs` and `/dw leave id`. Any allowed member can host. Organized setup pages through the full playable Toon list, saves each count, asks for the host's Toon, and requires **Post run**. **All Toons** requires confirmation before clearing fixed counts. Casual setup skips those forms; a Toon is optional when joining. `/hostrun` resumes an existing draft without changing its name or mode. `/dw run` restores private controls after expiry or restart.
...
Run data version 3 is separate from the filesystem wiki catalog. `migrate_runs` upgrades the empty version-1 namespace; `migrate_run_cards` preserves version-2 runs, source pins, audit and receipts while adding public projections. Run aggregates are bounded at 40 KiB. Back up the host database before upgrade. Older binaries cannot read version-3 records. Rollback must retain those records and disable run writes rather than downgrading their schema.
