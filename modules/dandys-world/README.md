...
Run rollback and recovery:

1. Stop accepting run changes with `oracle --config CONFIG module deactivate --module community.dandys-world --guild GUILD`. Preserve the current database and module artifacts; take a fresh `oracle --config CONFIG backup --output NEW_BACKUP_DIRECTORY` after writes are stopped. Keep the wiki store backup separately.
2. Prefer restoring the current compatible module artifact against the current database. A version-3 reader must not be used against version-4 run records. Do not rewrite record versions or restore a pre-upgrade backup over the current database.
3. For a restore rehearsal, use a separate configuration targeting an absent SQLite file or an empty isolated PostgreSQL database, then run `oracle --config RESTORE_CONFIG restore --backup BACKUP_DIRECTORY`. Restore rejects an occupied destination, creates a new deployment identity, and pauses mutations. The old source database remains intact.
4. Compare active and terminal rosters, receipt outcomes, pinned sources, schedules and publication intents before resuming. An older backup cannot contain later signups: retain the newer source and reconcile those differences explicitly. A backup checksum failure or corrupt aggregate is a recovery incident, not permission to discard a roster.
5. Reinstall the verified compatible artifact and explicitly resume only after checking permissions, module health and existing Discord message identities. Observe uncertain publications before any repost; only an authorized explicit repost repairs a confirmed missing card. Keep the original deployment stopped during cutover to prevent two writers.

Signup Toon lists show the current page and total choices. Use **Next Toons** and **Previous Toons** to reach the whole casual roster. Organized signup offers only the host-selected Toons with available places; a missing or full allocation is not an eligible signup choice.
