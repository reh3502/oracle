...
The preset fixes metadata retention at 14 days, excludes identified Oracle-origin events, coalesces equivalent metadata for 30 seconds, and bounds the delivery queue at 128. Retained metadata uses 337 hourly slots with at most 128 records per hour. Overload drops new records and later reports a bounded aggregate. Stable accepted event IDs are deduplicated within their retained hourly records. Maintenance events remove expired records, flush membership summaries, and flush due notifications. Without maintenance, status reports the last completed retention run instead of claiming the job is healthy.

Configuration requires a destination channel ID; `operator_note` is preserved by preset merging. The host validates destination visibility, send permissions, and current intent availability. `status` combines the module’s applied configuration and delivery/retention counters with fresh host configuration, subscription, and destination checks. Missing or failed checks remain unavailable. `probe` requests one safe synthetic notification through the host's durable effect journal. A verified probe is tied to its destination and configuration revision; it does not imply that a real moderation event was observed.
...
Enable the server members intent for the bot in Discord's developer settings. Configure the host's `discord.intents` as `["guilds", "guild_members", "guild_moderation"]`, then restart the host. The bot also needs View Audit Log permission for the moderate preset. Use a private staff text channel that the bot can view, send to, and read message history in. Ordinary member roles must not have access to that channel.
...
Administrative notifications use audit entries with an explicit actor. Entries attributed to Oracle are excluded. Raw channel, role, ban, and member-role observations often omit the actor; the module retains those as unattributed metadata and counts them, but does not send them as external administrative activity. The corresponding audit entry can supply an authoritative record without relying on arrival order or a target/time guess.

Join/leave summaries are observational totals and do not claim who caused a departure. Status distinguishes raw unattributed observations from audit entries with a missing actor. Discord cannot provide complete attribution for every observation; these counters expose that limit without inventing an actor or claiming a real moderation action from a synthetic probe.
